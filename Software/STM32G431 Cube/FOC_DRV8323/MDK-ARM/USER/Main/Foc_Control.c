...
void FOC_Main_Loop_H_Freq(void)
{
...
#if FOC_CLOSE_LOOP_MODE == MODE_OFF
    Motor_FOC.Vd = 0.00f;  // 示例：设置固定 Vd
		#if MOTOR_TYPE == HT4315
    Motor_FOC.Vq = (0.5 * Motor_FOC.Speed_Rpm / Motor_FOC.Speed_Rpm_Expect + 0.5)*13.5f;  // 示例：设置固定 Vq
		#elif MOTOR_TYPE == DJI_SNAIL_2305
		Motor_FOC.Vq = (0.8 * Motor_FOC.Speed_Rpm / Motor_FOC.Speed_Rpm_Expect + 0.2)*10.0f;  // 示例：设置固定 Vq
		#endif
    ...
    PID_SetRef(&Current_Iq_PID, Speed_Rpm);
